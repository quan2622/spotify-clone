...
const UpdateAlbumUserDialog = ({
  ...
  const handleUpdateInfo = async () => {
    ...
    if (info.name !== infoClone.name) formData.append("title", info.name);
    if (info.description !== infoClone.description)
      formData.append("description", info.description);
    if (isUpload && image) formData.append("image", image);
    ...
    const res = await axiosIntance.put(`albums/update/${albumId}`, formData, {
    ...
    if (res.data && res.data.EC === 0) {
      ...
      useMusicStore.setState({
        ...
        albumsUser: albumsUser.map((item) =>
          item._id === res.data.newData._id ? res.data.newData : item
        ),
      });
      setDialogOpen(false);
    } else {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="bg-zinc-900  border-zinc-700 max-h-[80vh] sm:max-w-xl overflow-auto">
        ...
        <div className="flex gap-4 px-3 pt-4 w-full">
          ...
          />
          <div
            className="w-[40%] flex items-center justify-between group rounded-lg overflow-hidden "
            ...
            <div className="w-[280px] h-[209px] shadow-xl rounded bg-black/30 flex flex-col items-center justify-center  text-stone-400 group-hover:text-stone-300">
              {imgURL !== "" ? (
                <img
                  src={imgURL}
                  alt="image-album"
                  className="w-full h-full object-center shadow-xl rounded-lg"
                />
              ) : (
                <>
          ...
          <div className="w-[60%] flex flex-col gap-3 text-white">
            <div className="mb-1">
              ...
              <Input
                className="bg-stone-900/50 border-zinc-700 focus-visible:ring-1 focus-visible:ring-zinc-500 focus-visible:outline-none"
                value={info.name}
            ...
            <div>
              ...
              <Textarea
                className="h-full border-zinc-700 focus-visible:ring-1 focus-visible:ring-zinc-500 focus-visible:outline-none"
        ...
        <DialogFooter className="p-3">
          <Button size={"lg"} onClick={handleUpdateInfo}>
            Save
          </Button>
        </DialogFooter>
      </DialogContent>
